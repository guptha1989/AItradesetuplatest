...
export default function DhanDashboardView() {
  const { spot, optionChain: storeChain } = useTradingStore();
  ...
  const [expiries, setExpiries] = useState(['2026-08-04', '2026-08-11', '2026-08-18', '2026-08-25', '2026-09-01']);
  ...
  const fetchDhanDashboard = async () => {
    setLoading(true);
    try {
      ...
      const res = await fetch(`${API_BASE}/market/dhan-chain-dashboard?${params}`).then(r => r.json());
      if (res.contracts) {
      ...
      console.error('Fetch Dhan dashboard error:', err);
    } finally {
      setLoading(false);
    }
  };

  // Fetch on parameter change & set 30-second background polling timer
  useEffect(() => {
    fetchDhanDashboard();
  ...
  // Realtime updates over Dhan WebSocket stream
  useEffect(() => {
  ...
  // Filter & Sort processed contracts
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', fontFamily: 'Inter, sans-serif' }}>
      ...
      {/* 1. Header Banner & Dhan Live Connection Status */}
      <div className="flex-between">
        <div>
          <h1 style={{ fontSize: '1.4rem', fontWeight: 800, color: 'var(--color-text-primary)', margin: 0 }}>
            ⚡ Dhan HQ Live Option Chain Dashboard
          </h1>
          <div style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', marginTop: '4px' }}>
            Streaming real-time Nifty derivatives data directly via Dhan HQ API v2 & WebSockets
          </div>
        </div>
      ...
      {/* 3. Parameter Banner Cards matching exact NSE / Dhan parameters */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '14px' }}>
        ...
        <div className="card" style={{ padding: '14px 18px', background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: '10px' }}>
          <span style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', fontWeight: 600 }}>Dhan Nifty Spot : </span>
      ...
      <div className="card" style={{ padding: '16px 20px', background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: '10px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
          ...
          {/* Active Tuesday Expiry Date Select */}
          <select
              ...
              {processedContracts.length === 0 ? (
                <tr>
                  <td colSpan={16} style={{ padding: '40px', textAlign: 'center', color: 'var(--color-text-muted)' }}>
                    {loading ? '⏳ Fetching live Dhan HQ API option chain...' : 'No Dhan contracts available for the selected parameters.'}
                  </td>
                </tr>
              ) : (
